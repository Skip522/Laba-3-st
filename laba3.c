...
// пара срока слобец (координаты клеток)
struct Point {
    int r, c;
};


// Очередь через динамический массив head и tail индексы начало и конца очереди (выделение памяти происходит один раз при создании)
...
// Очередь через связанные списки Каждый push — new Node, каждый pop — delete
// ListQueue хранит точки в цепочке узлов. Новые точки добавляются в конец через tail, а удаляются из начала через head
class ListQueue {
private:
    struct Node {
        Point pt;
...
public:
    ...
    Point front() { return head->pt; } // просмотр первого элемента
...
// алгоритм BFS (принимает любой тип очереди)

int dr[] = {-1, 1, 0, 0}; // строка
int dc[] = {0, 0, -1, 1}; // столбец
